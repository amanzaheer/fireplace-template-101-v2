...
export default function Navbar13({ content }) {
  ...
  const headerContent = (
    <>
      <div className="flex h-full w-full items-center justify-between md:pr-8">
        ...
        <div className="hidden items-center justify-center gap-6 rounded-full bg-[#d7d7d7] px-6 py-3 lg:flex xl:gap-8">
            ...
            >
              ...
              >
                <Image
                  ...
                  width={20}
                  height={20}
                  className="h-[40px] w-[60px] object-contain ml-4 "
                  aria-hidden
                />
              </span>
              <div
  ...
  return (
    <FullContainer id="navbar" className={headerShellClass}>
      ...
      >
        <div className={cn(inter.className, "flex flex-col text-[17px] font-semibold")}>
          {menuItemsArray.map((item) => {
            if (isDropdownItem(item)) {
              const children = getDropdownChildren(item);
              const dropdownKey = item.childrenRef ?? item.title;
