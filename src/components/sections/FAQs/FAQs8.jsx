...
export default function FAQs8({ content }) {
  ...
  return (
    <FullContainer className="py-6" id="faqs">
      <Container className="px-4">
        <div className="mx-auto max-w-full rounded-xl border border-white/10 bg-[#08285a] p-4 shadow-[0_12px_30px_rgba(0,0,0,0.25)] md:p-6">
            ...
            {faqItems.map((faq, index) => (
              <div key={faq?.id ?? index} className="mb-2">
                <button
                  type="button"
                  id={`faq-btn-${index}`}
                  className={`w-full text-left py-3.5 px-4 rounded-lg flex items-center focus:outline-none bg-orange-500 text-white`}
                ...
                >
                  <div className="bg-orange-100 p-4 border border-orange-200 border-t-0 rounded-b">
                    <p className="text-orange-950 text-start">{faq?.answer ?? faq?.a ?? ""}</p>
                  </div>
                </div>
              </div>
            ))}
