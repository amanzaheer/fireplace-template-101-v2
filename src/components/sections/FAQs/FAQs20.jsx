...
const ROW_BLUE = "#415FB0";
const CTA_RED = "#c41e3a";
const OPEN_QUESTION = "#9b2335";
...
export default function FAQs20({ content }) {
  ...
  return (
    <FullContainer className="bg-white py-10 md:py-14 lg:py-20" id="faqs">
      <Container className="px-4 sm:px-6 lg:px-8">
        ...
        >
          {/* Left: intro + CTA (centered) */}
          <div className="flex min-w-0 flex-col items-center text-center lg:max-w-lg lg:justify-self-center lg:self-center xl:max-w-xl">
            <h2 className="text-3xl font-extrabold leading-tight tracking-tight text-black md:text-4xl lg:text-[2.5rem] lg:leading-[1.15]">
              {faqTitle}
            </h2>
            {introText ? (
              <p className="mt-4 w-full max-w-lg text-base font-normal leading-relaxed text-black md:text-[17px]">
                {introText}
              </p>
            ) : null}
            {ctaLabel && ctaHref ? (
              <a
                href={ctaHref}
                className="mt-8 inline-flex min-h-[48px] w-fit items-center justify-center rounded-full px-10 py-3 text-sm font-bold uppercase tracking-wide text-white transition hover:brightness-105 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
                style={{
              ...
              return (
                ...
                >
                  <button
                    ...
                    id={`faq-button-${index}`}
                    className={cn(
                      "flex w-full items-center justify-between gap-4 px-5 py-4 text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#415FB0]/40 focus-visible:ring-offset-2 md:px-6 md:py-[18px]",
                      isOpen ? "bg-white font-bold" : "font-bold text-white",
                    )}
                  ...
                  >
                    <span
                      className="min-w-0 flex-1 pr-2 text-[16px] leading-snug md:text-[17px]"
                  ...
                  >
                    <div className="px-5 py-4 text-left text-[15px] font-normal leading-[1.65] text-black md:px-6 md:text-[16px] md:leading-7">
                      <p>{answer}</p>
                    </div>
                  </div>
                </div>
