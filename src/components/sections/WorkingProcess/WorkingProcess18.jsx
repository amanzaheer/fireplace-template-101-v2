...
import QuoteForm18 from "../Banner/QuoteForm/QuoteForm18";
...
export default function WorkingProcess18({ content }) {
  const data =
    ...
    content?.process ??
    {};
  const heading = data?.title ?? data?.heading ?? "Call for a free estimate";
  const description =
    data?.description ??
    "Expert chimney sweeping & fireplace care. Fully insured & trusted professionals.";
  const steps = Array.isArray(data?.list) ? data.list.slice(0, 3) : [];
  const formHead = {
    title: content?.banner?.form_title ?? data?.form_title ?? "GET IN TOUCH WITH US",
    sub_title: content?.banner?.form_description ?? data?.form_subtitle ?? "",
  };
  ...
  return (
    <FullContainer
      id="working_process"
      className={cn(
        "relative w-full items-stretch overflow-hidden bg-white p-0",
        poppins.className,
      )}
    >
      <div className="relative flex w-full flex-col lg:min-h-[520px] lg:flex-row">
        {/* Left ~58% — red panel, centered content */}
        <div className="order-2 flex w-full flex-col justify-center bg-[#FF0011] px-5 py-10 text-center text-white sm:px-8 sm:py-12 lg:order-1 lg:w-[58%] lg:px-10 lg:py-14 xl:px-14">
          <div className="mx-auto w-full max-w-3xl">
            <h2 className="text-2xl font-regular font-poppins capitalize  leading-tight sm:text-3xl md:text-[2rem] lg:text-[2.15rem] lg:leading-[1.15]">
              {heading}
            </h2>
            {description ? (
              <p className="mx-auto mt-4  max-w-2xl text-sm leading-relaxed text-white/95 sm:text-[15px] md:text-[16px] md:leading-[1.65]">
                {description}
              </p>
            ) : null}
          </div>
          <div className="mx-auto mt-8 grid w-full max-w-4xl grid-cols-1 gap-8 sm:grid-cols-3 sm:gap-5 md:mt-10 lg:gap-6">
            {steps.map((step, idx) => {
              const stepImage = step?.file_name ?? step?.image ?? "";
              const imageSrc = buildImageSrc(IMAGE_BASE, stepImage);
              ...
              return (
                ...
                >
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[#FAFAFA] shadow-sm">
                    {imageSrc ? (
                  ...
                  {step?.title ? (
                    <h3 className="mt-4 text-sm  md:text-[18px] font-bold leading-snug sm:text-base">
                      {step.title}
                    </h3>
                  ) : null}
                  {step?.description ? (
                    <p className="mt-2 text-[11px] md:text-[15px] leading-relaxed text-white/90 sm:text-xs">
                      {step.description}
                    </p>
                  ) : null}
                </article>
        ...
        {/* Right ~42% — white panel + form */}
        <div className="order-1 flex w-full items-center justify-center bg-white px-5 py-10 sm:px-8 sm:py-12 lg:order-2 lg:w-[42%] lg:py-14">
          <div id="quote-form-section" className="flex w-full justify-center">
            <QuoteForm18 data={content} form_head={formHead} variant="card" />
          </div>
        </div>
      </div>
    </FullContainer>
