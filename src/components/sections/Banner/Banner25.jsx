...
import QuoteForm25 from "./QuoteForm/QuoteForm25";
...
export default function Banner25({ content }) {
        ...
        >
            ...
            <Container className="relative z-10 font-barlow py-10 md:py-12 lg:py-16">
                <div className="w-full flex flex-col lg:flex-row items-center lg:items-start justify-between gap-8 lg:gap-12 text-white">
                    <div className="w-full max-w-[700px]">
                        <div className="w-fit flex flex-col items-start justify-center">
                            ...
                            >
                                ...
                                <br />
                                <span className="text-[#D90209] text-[60px] md:text-[74px]">
                                    {String(data?.heading || data?.title || "")
                                        .split(" ")
                                        .slice(4)
                                        .join(" ")}{" "}
                            ...
                            {data?.tagline ? (
                                <h2 className="text-lg md:text-2xl font-semibold leading-tight text-black/90 text-left mt-2">
                                    {data?.tagline}
                                </h2>
                            ) : null}

                            <p className="text-base md:text-lg text-left mt-3 mb-1 text-black/90 max-w-[560px]">
                                {data?.description}
                            </p>
                            {features?.length > 0 ? (
                                <ul className="mb-6 w-fit grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 md:gap-y-4">
                                    {features?.map((feature, idx) => {
                                        ...
                                        return (
                                            <li
                                                key={idx}
                                                className="flex items-center gap-2.5 leading-none text-white font-semibold text-[30px] md:text-[34px]"
                                            >
                                                {IconComponent ? (
                            ...
                            <div className="w-fit">
                                <a
                                    ...
                                    className="inline-flex items-center gap-3 rounded-full bg-transparent text-black text-[40px] md:text-[54px] font-extrabold leading-none"
                                >
                                    ...
                                    <span
                                        className={`${rubik.className} text-black font-bold text-[30px] md:text-[40px] uppercase`}
                                    >
                                        {phone}
                                    </span>
                                </a>
                    ...
                    <div className="w-full md:w-fit max-w-[470px] relative md:mt-20 ">
                        <div className="rounded-2xl shadow-[0_18px_50px_rgba(0,0,0,0.45)] overflow-hidden">
                            <QuoteForm25
                                data={data}
                                form_head={form_head}
