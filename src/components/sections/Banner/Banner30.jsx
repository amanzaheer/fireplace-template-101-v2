...
export default function Banner30({ content }) {
    ...
    return (
        ...
        >
            ...
            <Container className="relative z-10 font-barlow py-10 md:py-14 lg:py-12">
                <div className="w-full flex flex-col min-h-[400px]  items-center lg:items-start justify-between gap-8 lg:gap-10 text-black">
                    <div className="relative w-full max-w-full">
                        <div className="w-full flex flex-col items-start justify-center">
                            ...
                            {data?.tagline ? (
                                <h2 className="text-xl text-center  md:text-2xl uppercase font-extrabold leading-tight text-[#000000] mt-2 w-full">
                                    {data?.tagline}
                                </h2>
                            ) : null}

                            <p className="text-base md:text-lg text-left mt-3 mb-2 text-black max-w-[520px]">
                                {data?.description}
                            </p>
                            {features?.length > 0 ? (
                                <ul className="mb-6 flex flex-col lg:flex-row items-center pt-0 lg:pt-5 justify-center space-y-1 md:space-y-1.5 w-full gap-0 lg:gap-10">
                                    {features?.slice(0, 3).map((feature, idx) => {
                                        ...
                                        return (
                                            <li
                                                key={idx}
                                                className={`${inter.className} flex items-center gap-2.5 text-black font-normal text-lg md:text-2xl`}
                                            >
                                                {IconComponent ? (
