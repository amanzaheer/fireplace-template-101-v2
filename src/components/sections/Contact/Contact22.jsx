...
const NameInput = memo(({ value, onChange, error }) => (
  <div>
    <input
      ...
      value={value}
      onChange={onChange}
      className={`w-full px-5 py-3 text-base md:text-lg border  border-black rounded bg-transparent outline-none text-black placeholder:text-black ${error ? "border-red-500" : "border-black"} focus:border-gray-300`}
...
const EmailInput = memo(({ value, onChange, error }) => (
  <div>
    <input
      ...
      value={value}
      onChange={onChange}
      className={`w-full px-5 py-3 text-base md:text-lg border  bg-transparent outline-none rounded text-black placeholder:text-black ${error ? "border-red-500" : "border-black"} focus:border-gray-300`}
...
const ZipcodeInput = memo(({ value, onChange, error }) => (
  <div>
    <input
      ...
      value={value}
      onChange={onChange}
      className={`w-full px-5 py-3 text-base md:text-lg border  bg-transparent outline-none text-black rounded placeholder:text-black ${error ? "border-red-500" : "border-black"} focus:border-gray-300`}
...
const MessageInput = memo(({ value, onChange, error }) => (
  <div>
    <textarea
      ...
      onChange={onChange}
      rows={4}
      className={`w-full px-5 py-3 text-base md:text-lg border bg-transparent outline-none text-black rounded placeholder:text-black min-h-[70px] ${error ? "border-red-500" : "border-black"} focus:border-gray-300`}
      placeholder="Message"
      required
      aria-invalid={!!error}
    />
...
export default function Contact22({ content }) {
    ...
    <FullContainer id="contact-us" className="pb-4 relative mt-9 ">
      <Container className="relative z-10">
        <div id="quote-form-section">
          <div className=" gap-0 overflow-hidden mb-5  w-full">
            <div className="p-5 md:p-8 lg:p-10 text-black">
              {formSubmitted ? (
                ...
                <>
                  <h2 className="text-center text-4xl md:text-[52px] font-bold text-black tracking-tight">
                    {title}
                  </h2>
                  <h3 className="text-4xl md:text-[52px] font-bold mb-6 md:mb-8 text-black text-center leading-tight">
                    {subTitle}
                  </h3>
                  <form
                  ...
                  >
                    ...
                    <div className="pt-2">
                      <button
                        ...
                        className="w-[210px] h-[55px] mx-auto bg-black text-white transition-all duration-300 font-medium text-xl flex items-center justify-center hover:bg-[#212020] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 focus:ring-offset-[#1f2226] disabled:opacity-70"
