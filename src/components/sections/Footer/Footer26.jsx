    ...
    return function FooterLeafletMap({ center, markers, zoom = 11 }) {
      return (
        <leaflet.MapContainer
          ...
          className="h-[280px] w-full lg:h-[300px]"
        >
          <leaflet.TileLayer
...
export default function Footer26({ content }) {
  ...
  const callNowLabel =
    (typeof footer.call_now_label === "string" && footer.call_now_label.trim()) ||
  ...
  const mapMarkers = useMemo(() => {
    ...
    if (manualMarkers.length) {
      return manualMarkers
        ...
        .map((marker) => ({
          ...
          label: marker?.label ?? mapAddress ?? address ?? "",
        }));
    }
    ...
    const locationLabel = mapAddress || address || "";
  ...
  const renderNav = () =>
    menuItems.length > 0 ? (
      <nav
        className="flex w-full flex-col items-start gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-5 sm:gap-y-2 lg:justify-end lg:gap-x-7"
        aria-label="Footer navigation"
      >
        {menuItems.map((item, navIndex) => {
          const navKey = item?.title ?? item?.link ?? `footer-nav-${navIndex}`;
  ...
  return (
    <footer
      className={`${montserrat.className} relative overflow-visible bg-black text-white antialiased`}
    >
      <FullContainer
        id="footer"
        className="relative z-0 overflow-visible bg-black py-10 pb-[max(2.5rem,env(safe-area-inset-bottom,0px))] md:py-12 lg:py-14"
      >
        <Container className="mx-auto max-w-[1440px] px-4 sm:px-6 md:px-8 lg:px-[170px]">
          <div className="grid grid-cols-1 gap-8 sm:gap-10 lg:grid-cols-[1fr_1fr_0.95fr] lg:grid-rows-[auto_1fr] lg:gap-x-8 lg:gap-y-10 xl:gap-x-12">
            {/* Column 1 — brand, badges, CTA */}
            <div className="flex flex-col items-start gap-5 lg:row-span-2 lg:gap-6">
              {navbar?.logo ? (
                <Logo
                  logo={navbar.logo}
              ...
              {description ? (
                <p className="max-w-[360px] text-sm leading-relaxed text-white/85 md:text-[15px]">
                  {description}
                </p>
              ) : null}
              ...
              {phone && callNowLabel ? (
                <Link
                  href={telHref(phone)}
                  title={callNowLabel}
                  className="inline-flex w-full max-w-[340px] items-center gap-4 rounded-full bg-[#D32F2F] px-5 py-3.5 text-white shadow-md transition hover:brightness-110 sm:max-w-[360px] sm:px-6 sm:py-4"
                >
            ...
            <div
              className={cn(
                "flex flex-col",
                menuItems.length > 0 ? "lg:col-start-2 lg:row-start-2" : "lg:col-start-2",
              )}
            >
              <h3 className="text-xl font-bold text-white md:text-2xl">
                {contactHeading}
              </h3>
              <ul className="mt-5 space-y-4 text-sm text-white/90 md:mt-6 md:text-[15px]">
                {address ? (
                ...
                <div className="mt-6 flex flex-wrap gap-4 md:mt-8">
                  {socialLinks.map(({ label, href, Icon }) => (
                    <a
                      ...
                      aria-label={label}
                      className="text-[#D32F2F] transition hover:text-[#ff6b6b]"
                    >
            ...
            <div
              className={cn(
                ...
                menuItems.length > 0 ? "lg:col-start-3 lg:row-start-2" : "lg:col-start-3",
              )}
            >
              <div className="mx-auto h-[240px] w-full max-w-full overflow-hidden rounded-2xl border border-white/10 bg-zinc-900 shadow-lg sm:h-[280px] sm:max-w-[320px] lg:mx-0 lg:h-[300px] lg:max-w-[300px] xl:max-w-[320px]">
                <LeafletMap
                  center={mapCenter}
                  markers={mapMarkers}
              ...
              {mapAddress ? (
                <p className="mt-4 flex max-w-[320px] items-start gap-2.5 text-sm leading-relaxed text-white/90 md:text-[15px] lg:max-w-[300px] xl:max-w-[320px]">
                  <MapPin
                    className={cn("mt-0.5 h-5 w-5 shrink-0", iconAccent)}
                    strokeWidth={2}
                    aria-hidden
