...
export default function Footer21({ content }) {
  ...
  const stripHeading =
    strip?.heading ?? "It's Easy! Call Now.";
  const stripPhoneLabel = strip?.phone_label ?? "CALL NOW:";
  const stripPhone =
    strip?.phone ??
    banner.cta_phone ??
    contactInfo.phone ??
  ...
  return (
    <footer id="footer">
      <div className="relative z-20 overflow-x-hidden bg-[#ffffff]">
        <div className="w-full min-w-0 px-3 sm:px-4 md:px-0">
          <div className="mx-auto w-full min-w-0 max-w-[1190px]">
            <div
              className="mx-auto box-border h-[10px] shrink-0 bg-[#082A51]"
              style={{ width: "95.3%", maxWidth: 1134 }}
              aria-hidden
            />
            <div className="relative min-h-[118px] overflow-hidden bg-[#F86503] px-4 py-5 sm:min-h-[128px] sm:px-6 sm:py-6 md:min-h-[142px] md:px-8 md:py-7 lg:min-h-[150px] lg:px-10 lg:py-8 [clip-path:polygon(2.5%_0%,97.5%_0%,100%_100%,0%_100%)]">
              <div className="flex w-full min-h-0 flex-col items-stretch justify-center gap-5 text-center sm:gap-6 md:flex-row md:items-center md:justify-between md:gap-8 md:text-left">
                ...
                <div className="flex w-full shrink-0 justify-center md:w-auto md:justify-end">
                  <a
                    href={
                      stripPhone
                        ? `tel:${String(stripPhone).replace(/[^\d+]/g, "")}`
                        : "#"
                    }
      ...
      <FullContainer className="relative mb-16 bg-[#082A51] py-12 md:mb-0 md:py-16">
          <Container className={`relative z-10 ${archivo.className}`}>
            <div className="grid grid-cols-1 gap-10 md:grid-cols-3 md:gap-12">
            {/* Column 1 — Logos + Statement */}
            <div>
