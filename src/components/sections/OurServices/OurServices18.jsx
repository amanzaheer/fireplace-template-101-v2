...
const BUTTON_CLASS =
  "inline-flex min-h-[40px] min-w-[140px] items-center justify-center rounded-md bg-[#FF1E1E] px-6 text-sm font-bold uppercase tracking-wide text-white transition-colors duration-200 hover:bg-[#e01515]";
...
function ServiceCard({ service, buttonLabel }) {
  ...
  return (
    <article className="flex min-h-[163px] flex-col items-center justify-center rounded-2xl bg-white px-5 py-6 text-center shadow-sm transition-shadow duration-200 hover:shadow-md">
      <h3 className="mb-2 font-poppins text-xl font-medium leading-tight text-black md:text-[22px]">
        {service.title}
      </h3>
      {service.description ? (
        <div
          className="mb-4 font-poppins text-[13px] font-normal leading-relaxed text-black [&_a]:text-black [&_p]:m-0 [&_p]:font-poppins [&_p]:text-[13px] [&_strong]:font-semibold"
...
export default function OurServices18({ content }) {
  ...
  const buttonLabel =
    typeof ourServices?.cta_label === "string" && ourServices.cta_label.trim()
      ...
      : "VIEW MORE";
  return (
    <FullContainer id="our_services" className="bg-black py-12 md:py-16">
      <Container>
        <h2 className="mb-4 text-center text-4xl font-medium font-poppins tracking-tight text-white md:text-[44px]">
          {title}
        </h2>
        {description ? (
