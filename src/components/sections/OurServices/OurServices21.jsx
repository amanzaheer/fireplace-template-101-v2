...
const CARD_CONTENT_TEXT_CLASS =
  "max-w-full text-left text-[14px] font-normal leading-[1.55] text-[#656565]";
...
export default function OurServices21({ content }) {
  ...
  const sectionSub =
    ...
    "";
  return (
    <FullContainer id="our_services" className="bg-[#ffffff] py-10 md:py-14 lg:py-16">
      <Container className="px-4 sm:px-6 lg:px-8">
        <header className="mx-auto mb-8 max-w-4xl text-center md:mb-10">
          <h2
            className={`${poppins.className} text-center text-[34px] font-semibold leading-[1.15] text-[#1f1f1f] capitalize md:text-[44px]`}
          >
            {title}
          </h2>
          {sectionSub ? (
            <p className="mx-auto mt-3 max-w-3xl text-sm leading-relaxed text-[#555] sm:text-base md:mt-4 md:text-[17px]">
              {sectionSub}
            </p>
          ) : null}
        </header>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3 lg:gap-7">
          {displayServices.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className="flex h-full flex-col overflow-hidden rounded-[14px] border border-[#e4e4e4] bg-white shadow-[0_6px_18px_rgba(0,0,0,0.08)] transition-shadow duration-200 hover:shadow-[0_10px_24px_rgba(0,0,0,0.12)]"
              >
                ...
                <div className="flex flex-1 flex-col gap-3 p-5 md:p-6">
                  <h3 className="text-left text-[24px] font-semibold leading-tight text-[#252525]">
                    {service.path && service.path !== "#" ? (
                      <Link
                        href={service.path}
                        className="text-[#252525] transition-colors hover:text-[#F0520E]"
                      >
                        {service.title}
                      </Link>
                    ) : (
                  ...
                  {service.description ? (
                    <div
                      className={`flex-1 ${poppins.className} ${CARD_CONTENT_TEXT_CLASS} **:font-inherit **:text-[#656565] **:leading-[1.55] [&_p]:m-0 [&_ul]:m-0 [&_ol]:m-0 [&_li]:m-0`}
                  ...
                  <a
                    ...
                    className="mt-auto inline-flex w-fit items-center justify-center gap-2.5 rounded-[10px] bg-[#F0520E] px-6 py-3 text-[13px] font-bold uppercase tracking-wide text-white transition-colors hover:bg-[#d9480c]"
                  >
