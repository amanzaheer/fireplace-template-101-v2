    ...
    <FullContainer id="service_benefits" className="py-10 md:py-14 overflow-hidden">
      <Container>
        <div className="grid grid-cols-1 items-stretch gap-6 lg:grid-cols-[1.05fr_1fr] lg:gap-8">
          ...
          <div className="flex flex-col rounded-[24px] border border-black/5 bg-white p-5 shadow-[1px_4px_15px_0_rgba(0,0,0,0.13)] md:p-8">
            {heading ? (
            ...
            {benefits.length ? (
              <ul className="mt-5 space-y-3 md:mt-6">
                {benefits.map((benefit, index) => (
                  ...
                  >
                    <span className="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#F59402]/15 text-[#F59402]">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
              {phone ? (
                <Link
                  href={tel}
                  className={`${poppins.className} inline-flex h-[50px] items-center justify-center gap-2 rounded-[10px] bg-[#F59402] px-5 text-[17px] font-semibold text-white transition-colors hover:bg-[#df8601]`}
                >
                  <PhoneCall className="h-4 w-4" strokeWidth={2.5} aria-hidden />
                  <span>{phone}</span>
                </Link>
              ) : null}
              ...
              <Link
                href={phone ? tel : "#"}
                className={`${inter.className} inline-flex h-[50px] items-center justify-center gap-2 rounded-[10px] bg-[#786F6F] px-6 text-sm font-semibold uppercase tracking-wide text-white transition-colors hover:bg-[#6d6565]`}
              >
