...
export default function ServiceBenefits26({ content }) {
  const phone =
  ...
  const ctaLines = ctaLabel.includes("|")
    ? ctaLabel.split("|").map((line) => line.trim()).filter(Boolean)
    : ctaLabel
      ? [ctaLabel]
      : [];
  const imageAlt =
  ...
  return (
    <FullContainer id="service_benefits" className="bg-[#D90209] py-16 lg:py-20 text-white overflow-hidden">
      <Container>
          ...
          <div className="flex flex-col space-y-8 lg:space-y-12">
            ...
            {phone ? (
              <div>
                ...
                >
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#D90209] text-white">
          ...
          <div className="flex flex-col space-y-8 lg:space-y-10">
            ...
            {ctaLines.length > 0 ? (
              <div>
                <Link
                  href={phone ? tel : "#"}
                  className={`${poppins.className} w-full inline-flex min-h-[64px] py-3 flex-col items-center justify-center rounded-[18px] bg-white text-[#000000] px-6 transition-transform duration-300 hover:scale-[1.02] shadow-2xl text-center`}
                >
                  {ctaLines.length > 1 ? (
                    <>
