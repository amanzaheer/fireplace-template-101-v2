      ...
      <Container>
        <div className={`mx-auto grid max-w-[1220px] grid-cols-1 gap-8 md:grid-cols-[minmax(0,1fr)_356px_minmax(0,1fr)] md:items-center md:gap-8 ${poppins.className}`}>
          <div className="order-1 w-full ml-8 max-w-[380px] gap-4 justify-self-center md:justify-self-end">
            ...
            {sectionTitle ? (
              <h3 className="mb-8 text-[36px] font-bold leading-[1.02] text-black">
                {sectionTitle}
              </h3>
            ) : null}
            {intro ? (
              <p className="mb-4 max-w-[95%] text-[16px] leading-tight text-black">
                {intro}
              </p>
            ) : null}
            ...
            {phone ? (
              <a
                href={`tel:${phone}`}
                className="inline-flex h-[54px] w-[217px] flex-nowrap items-center justify-center gap-2 rounded bg-[#ff4b4d] px-4 text-[19px] leading-none text-white whitespace-nowrap"
              >
                <Image
