...
const CheckIcon = () => (
  <span
    className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 border-white text-white"
    aria-hidden
  >
...
const PromotionCard = ({
  ...
  return (
    <div
      className={`relative flex h-full min-h-[500px] flex-col overflow-hidden rounded-lg border-2 border-b-0 border-white bg-[#da4909] p-6 text-white transition-all duration-200${
        isMainCard ? "pt-6" : ""
      }`}
    >
      {heading && (
      ...
      >
        {(Array.isArray(features) ? features : [])?.map((feature, index) => (
          <div
            key={index}
            className={`${montserrat.className} flex w-full items-start gap-2 text-xs md:text-sm font-medium text-white leading-snug`}
          > 
            <CheckIcon />
      ...
      <button
        className={`${montserrat.className} -mx-[calc(1.5rem+2px)] -mb-6 mt-auto flex h-[50px] w-auto items-center justify-center gap-2 bg-black px-6 text-white font-extrabold text-xl tracking-wide uppercase transition-all duration-200 hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:ring-offset-2`}
      >
        {isMainCard ? (
...
export default function Promotion12({ content }) {
