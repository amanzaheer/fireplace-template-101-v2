...
const PromotionCard = ({
    ...
    return (
        <div
            className={`relative flex h-full min-h-[500px] flex-col overflow-hidden border-x-2 border-black bg-white p-6 text-black transition-all duration-200 ${isMainCard ? "pt-6" : ""
                }`}
        >
            {/* Header */}
            {heading && (
            ...
            {/* Features */}
            <div
                className={`flex flex-1 flex-col items-center gap-3 w-full ${isMainCard ? "mt-8" : isLastCard ? "mt-6" : "mt-3"
                    }`}
            >
...
export default function Promotion29({ content }) {
        ...
        <FullContainer id="promo">
            <Container>
                <div className="w-full pb-12 pt-8">
                    ...
                    {/* Cards */}
                    <div className="grid w-2/3 grid-cols-1 gap-8 md:grid-cols-2 md:gap-6 md:max-w-3xl md:mx-auto">
                        {details
