...
const PromotionCard = ({
    ...
    return (
        <div
            className={`relative flex flex-col h-full min-h-[450px] border-2 border-t-0  border-b-0 border-black rounded-t-3xl bg-white  p-6 text-black transition-all duration-200${isMainCard ? "pt-6" : ""
                }`}
        >
            {heading && (
                <div className="-mx-6 -mt-6 mb-3 flex h-[132px] w-auto flex-col rounded-t-3xl justify-center bg-[#000000] px-6 py-4 text-center text-white">
            ...
            <div
                className={`flex flex-col items-center gap-3 flex-1 w-full ${isMainCard ? "mt-8" : isLastCard ? "mt-6" : "mt-3"
                    }`}
            >
...
export default function Promotion25({ content }) {
        ...
        <FullContainer id="promo">
            <Container>
                <div className="w-full pb-12 pt-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-6 justify-center items-center rounded-t-xl w-3/4 mx-auto">
                        {(() => {
                            const first = 0;
                            const last = Math.max(0, details.length - 1);
                            const indices = first === last ? [first] : [first, last];
                            return indices.map((origIndex) => {
                                const item = details[origIndex];
                                if (!item) return null;
                                const isMain = origIndex === 1;
                                const isLast = origIndex === details.length - 1;
                                return (
                                    <PromotionCard
                                        key={origIndex}
