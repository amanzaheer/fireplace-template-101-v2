...
const CheckIcon = ({ isMainCard }) => (
    <Icon
        ...
        size={1.2}
        color={isMainCard ? "#ffffff" : "#FF0011"}
...
const PromotionCard = ({
    ...
    return (
        <div
            className={`relative flex flex-col h-full rounded-[22px] p-6 md:p-7 border shadow-sm mt-6 ${isMainCard
                    ? "bg-[#FF0011] text-white border-[#f59403] md:scale-[1.02]"
        ...
        >
            {heading && (
                <div className="mb-2">
                    <MaybeMarkdown
                        as="h3"
                        className={`${poppins.className} font-extrabold tracking-tight leading-tight ${isMainCard
            ...
            <div className="space-y-2.5 flex-1 mt-4">
                {(Array.isArray(features) ? features : [])?.map((feature, index) => (
                    <div
                        key={index}
                        className={`flex items-center gap-1.5 text-[30px] leading-snug ${isMainCard ? "text-white/95" : "text-[#212020]"
                            }`}
                    >
            ...
            {isMainCard ? (
                <a
                    ...
                    className="mt-2 inline-flex w-fit mb-4 items-center gap-2 rounded-xl self-center bg-white text-[#CC3333] font-semibold px-5 py-2.5 text-[28px] leading-none"
                >
                ...
                <a
                    href={phone ? `tel:${phone}` : "#"}
                    className={`${rubik.className} mt-6 inline-flex w-fit items-center gap-2 rounded-xl self-center bg-[#FF0011] text-white font-semibold uppercase px-6 py-3 md:px-7 md:py-4 text-[12px] md:text-[14px] leading-none`}
                >
                    Call Us Today
...
export default function Promotion27({ content }) {
    ...
    return (
        <FullContainer id="promo" className="py-10 md:py-14">
            <Container>
