...
const CheckIcon = ({ filled }) => (
  ...
  >
    <Icon
      ...
      size={4}
      className={filled ? "text-[#f59402]" : "text-[#ffffff]"}
    />
  </span>
);
    ...
    >
      ...
      <div className="space-y-3 flex-1">
        {(Array.isArray(features) ? features : [])?.map((feature, index) => (
          <div
            key={index}
            className={`${inter.className} flex items-start gap-2 text-sm md:text-[16px] font-medium ${
      ...
      <button
        className={`${rubik.className} w-full  flex items-center justify-center gap-2 mt-6 py-2  font-bold text-xl tracking-wide transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:ring-offset-2 ${
          isMainCard
            ? "bg-white text-[#f59402] hover:bg-blue-100"
            : "bg-[#f59402] text-white hover:bg-[#efa536]"
        }`}
      >
        {isMainCard ? 
        ...
        : 
        <span className={`${rubik.className} text-xs md:text-sm py-2 px-4 flex items-center justify-center gap-2`}>  
...
export default function Promotion32({ content }) {
