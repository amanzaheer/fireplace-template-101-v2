...
const CheckIcon = ({ isMainCard }) => (
  <Icon
    ...
    size={1.2}
    color={isMainCard ? "#ffffff" : "#F86503"}
...
const PromotionCard = ({
  ...
  return (
    <div
      className={`relative flex flex-col h-[529px] p-6 md:p-7 border shadow-sm mt-6 ${
        isMainCard
          ? "bg-[#F86503] text-white border-[#f59403] md:scale-[1.02] rounded-t-[12px] rounded-b-none"
          : "bg-white text-[#121212] border-[#e7e7e7] rounded-[12px]"
      }`}
    >
      {heading && (
        <div className="mb-2">
      ...
      {isMainCard ? (
        <a
          ...
          className="mt-2 inline-flex w-fit mb-4 items-center gap-2 rounded-md bg-white text-[#F86503] font-semibold px-5 py-2.5 text-[28px] leading-none"
        >
        ...
        <a
          href={phone ? `tel:${phone}` : "#"}
          className={`${rubik.className} mt-6 inline-flex w-fit items-center gap-2 rounded-md bg-[#F86503] text-white font-semibold uppercase px-6 py-3 md:px-7 md:py-4 text-[12px] md:text-[14px] leading-none`}
        >
          Call Us Today
...
export default function Promotion21({ content }) {
  ...
  return (
    <FullContainer id="promo" className="py-10 md:py-14 bg-[#ffffff]">
      <Container>
        <div className="w-full">
          <MaybeMarkdown
            as="h2"
            className={`${poppins.className} text-4xl md:text-[44px] font-bold text-center text-[#212020] mb-8 md:mb-10 tracking-tight`}
          >
            {title}
          </MaybeMarkdown>
          {description && (
